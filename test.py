...
async def async_generate(prompt: str, model: str, temperature: float, max_tokens: int) -> tuple:
    ...
    client = AsyncOpenAI(
        ...
        base_url="https://openrouter.ai/api/v1"  # Critical: points to OpenRouter
    )
    ...
    try:
        ...
        # Optional: add retry logic here for rate limits
...
class LLMOnlyTranslator(BaseTranslator):
    def __init__(self, model="MODEL_NAME", name="LLMOnlyTranslator", temperature=0.7, max_tokens=512):
...
class EnhancedCritiqueAgent:
    def __init__(self, model="MODEL_NAME", temperature=0.7, max_tokens=512):
        self.model = model
        self.temperature = temperature
